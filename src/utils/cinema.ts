...
const cinemaHoverPreviewHandlers: Array<{
  evt: string;
  fn: (e: Event) => void;
}> = [];
...
const cinemaIntro = (() => {
  ...
  return {
    ...
    reset() {
      shownThisActivation = false;
    },
  };
})();
declare global {
  interface Window {
    optubeForceSpotlight?: () => void;
    __optubeCinemaHoverMO?: MutationObserver;
  }
}
...
function ensureCinemaHoverPreviewBlocker() {
  ...
  evts.forEach((evt) => {
    const fn = (e: Event) => {
      if (!document.documentElement.hasAttribute(ATTR)) return;
      if (!document.body.classList.contains("cinematic-home")) return;
      if (!document.body.classList.contains("cinema-hide-hover-preview"))
        return;
  ...
  const mo = new MutationObserver((muts) => {
    if (
      ...
      !document.body.classList.contains("cinema-hide-hover-preview")
    )
      return;
    for (const m of muts) {
...
function teardownCinemaHoverPreviewBlocker() {
  if (!cinemaHoverPreviewBlockerAttached) return;
  cinemaHoverPreviewHandlers.forEach((h) =>
...
export function setCinemaMode(on: boolean) {
  ...
  if (on) {
    ...
    document.body.classList.remove(
      "cinema-spotlight-half",
      "cinema-spotlight-quarter",
    );
    ...
    if (stateChanged) {
      ...
      try {
        localStorage.removeItem("optube_cinema_active");
      } catch {
        /* ignore */
      }
    }
...
export function applyCinema(settings: {
...
export function injectCinemaCSS() {
  ...
  html[${ATTR}] body.cinematic-home { overflow:hidden ; background:var(--netflix-black); }

  
  
  html[${ATTR}] body.cinematic-home ytd-app { --ytd-mini-guide-width:0px ; }
  ...
  html[${ATTR}] body.cinematic-home ytd-rich-grid-renderer[is-two-columns] { max-width:100vw ; }
  html[${ATTR}] body.cinematic-home ytd-two-column-browse-results-renderer { padding:0 ; }
  
    html[${ATTR}] body.cinematic-home #chips-wrapper.ytd-feed-filter-chip-bar-renderer { margin:0 ; padding:12px 48px ; width:100vw ; box-sizing:border-box ; position:relative; z-index:2100; }
  html[${ATTR}] body.cinematic-home ytd-masthead { background:transparent; border-bottom:none; }
  html[${ATTR}] body.cinematic-home #background.ytd-masthead { background:transparent; }
  html[${ATTR}] body.cinematic-home ytd-masthead #logo-icon { fill:var(--netflix-red); }
  html[${ATTR}] body.cinematic-home html[dark] { background:var(--netflix-black); }
    ...
    overflow-y:hidden ;
    position:fixed ;
    top: var(--cinema-spotlight-height) ;
    left:0 ;
    width:100vw ; /* ensure full viewport width */
    height: var(--cinema-carousel-height) ;
    max-width:100vw ; /* override inline max-width from YT */
    ...
    /* Ensure carousel stays below spotlight without overlapping meta content */
            html[${ATTR}] body.cinematic-home.cinema-spotlight-active ytd-rich-grid-renderer #contents.ytd-rich-grid-renderer {
                ...
                padding-top: 12px ;
                top: calc(var(--cinema-spotlight-height) + 12px) ; /* ensure clear separation */
                transition: none ; /* prevent visual snapping */
            }
            ...
            html[${ATTR}] body.cinematic-home.cinema-spotlight-active #optube-cinema-spotlight .optube-spotlight-meta { 
                padding-bottom: 48px ; /* extra padding to keep Watch button clear */
                z-index: 1300 ; /* ensure meta is above carousel */
            }
            ...
            html[${ATTR}] body.cinematic-home ytd-rich-grid-renderer #contents.ytd-rich-grid-renderer ytd-continuation-item-renderer { min-height: var(--cinema-carousel-height) ; }
        html[${ATTR}] body.cinematic-home.cinema-spotlight-active ytd-rich-grid-renderer #contents.ytd-rich-grid-renderer::before { content:none ; }
        html[${ATTR}] body.cinematic-home #contents.ytd-rich-grid-renderer::after { content:none ; background:none ; }
        ...
        html[${ATTR}] body.cinematic-home ytd-rich-grid-renderer #contents.ytd-rich-grid-renderer > ytd-rich-item-renderer { position:relative ; z-index:3 ; }
  ...
  html[${ATTR}] body.cinematic-home ytd-rich-grid-renderer #contents.ytd-rich-grid-renderer[style] {
    ...
    margin-left:0 ;
    margin-right:0 ;
    top: calc(var(--cinema-spotlight-height) + 12px) ;
  }
  
  html[${ATTR}] body.cinematic-home ytd-rich-grid-renderer {
    --ytd-rich-grid-content-max-width: 100vw ;
  ...
  html[${ATTR}] body.cinematic-home ytd-rich-grid-renderer #contents.ytd-rich-grid-renderer > ytd-rich-item-renderer {
    ...
    html[${ATTR}] body.cinematic-home ytd-rich-grid-renderer #contents.ytd-rich-grid-renderer > ytd-rich-item-renderer.cinema-loading { opacity:0 ; transform:translateY(28px); }
  
  html[${ATTR}] body.cinematic-home ytd-rich-grid-renderer { --ytd-rich-grid-items-per-row: 1 ; }
  html[${ATTR}] body.cinematic-home ytd-rich-item-renderer { height:var(--cinema-carousel-height); aspect-ratio:1; margin:0; }
  html[${ATTR}] body.cinematic-home ytd-rich-item-renderer ytd-thumbnail { border:2px solid transparent; transition:top .2s, scale .2s, opacity .2s; scale:.95; position:relative; top:0; }
  html[${ATTR}] body.cinematic-home ytd-rich-item-renderer:hover ytd-thumbnail { scale:.85; top:-20px; opacity:.6; }
  html[${ATTR}] body.cinematic-home ytd-rich-grid-media #details { padding:1rem; background:var(--netflix-dark-gray); border-radius:4px; margin-top:-4px; transition:all .2s; opacity:0; top:-50px; position:relative; }
  html[${ATTR}] body.cinematic-home ytd-rich-grid-media:hover #details { opacity:1; top:-80px; }

  
  html[${ATTR}] body.cinematic-home #video-preview-container.ytd-video-preview { pointer-events:none ; }
  html[${ATTR}] body.cinematic-home ytd-video-preview { pointer-events:none ; }
  ...
  html[${ATTR}] body.cinematic-home #rich-shelf-header { display:none; }
    html[${ATTR}] body.cinematic-home #frosted-glass { display:none ; }
    ...
    html[${ATTR}] body.cinematic-home.cinema-hide-sidebar #guide-button,
        html[${ATTR}] body.cinematic-home.cinema-hide-sidebar ytd-app #guide { display:none ; }
    html[${ATTR}] body.cinematic-home.cinema-hide-shorts ytd-rich-shelf-renderer[is-shorts],
    html[${ATTR}] body.cinematic-home.cinema-hide-shorts ytd-reel-shelf-renderer { display:none ; }
    html[${ATTR}] body.cinematic-home.cinema-hide-create-btn ytd-topbar-menu-button-renderer:has(a[href*='studio.youtube.com']),
        html[${ATTR}] body.cinematic-home.cinema-hide-create-btn ytd-button-renderer:has(path[d*='M10 8v6l5-3-5-3z']) { display:none ; }
        
        html[${ATTR}] body.cinematic-home.cinema-hide-notifications ytd-notification-topbar-button-renderer { display:none ; }
    html[${ATTR}] body.cinematic-home.cinema-hide-searchbar ytd-masthead #center,
        ...
        html[${ATTR}] body.cinematic-home.cinema-hide-searchbar ytd-masthead form { display:none ; }
    ...
    html[${ATTR}] body.cinematic-home.cinema-hide-posts ytd-rich-item-renderer:has(> #content > ytd-post-renderer) { display:none ; }
    html[${ATTR}] body.cinematic-home.cinema-hide-badges-chips #chips-wrapper,
    html[${ATTR}] body.cinematic-home.cinema-hide-badges-chips ytd-feed-filter-chip-bar-renderer { display:none ; }
    ...
    html[${ATTR}] body.cinematic-home.cinema-hide-hover-preview yt-lockup-view-model:hover video { display:none ; }
    html[${ATTR}] body.cinematic-home.cinema-hide-hover-preview ytd-rich-item-renderer [class*='inline-preview'],
    html[${ATTR}] body.cinematic-home.cinema-hide-hover-preview ytd-rich-item-renderer [class*='InlinePreview'] { display:none ; }
    html[${ATTR}] body.cinematic-home.cinema-hide-masthead ytd-masthead { display:none ; }
        
        html[${ATTR}] body.cinematic-home.cinema-hide-hover-preview #video-preview-container.ytd-video-preview { display:none ; visibility:hidden ; }
    
    html[${ATTR}] body.cinematic-home.cinema-hide-spinners ytd-continuation-item-renderer { display:block ; opacity:1 ; }
    ...
    html[${ATTR}] body.cinematic-home.cinema-hide-spinners .ytp-spinner { opacity:0 ; visibility:hidden ; pointer-events:none ; animation:none ; }
    
    html[${ATTR}] body.cinematic-home.cinema-hide-spinners .yt-core-skeleton-loader { opacity:.35 ; filter:grayscale(1) brightness(.55); transition:opacity .3s ease; height: var(--cinema-carousel-height) ; }
    
    html[${ATTR}] body.cinematic-home ytd-continuation-item-renderer { min-height: var(--cinema-carousel-height) ; box-sizing:border-box; }
  ...
  html[${ATTR}] body.cinematic-home #contents:has(ytd-rich-item-renderer)::after { right:0; background:linear-gradient(to left,#0f0f0f,transparent); }
    ...
    html[${ATTR}] body.cinematic-home .cinema-scroll-arrow { cursor:pointer ; }
        html[${ATTR}] body.cinematic-home #contents .cinema-arrow-overlay .cinema-scroll-arrow:hover { background:rgba(0,0,0,0.66); opacity:1; }
            ...
            html[${ATTR}] body.cinematic-home #contents .cinema-arrow-overlay .cinema-scroll-arrow:hover { cursor:pointer; }
                ...
                html[${ATTR}] body.cinematic-home #contents .cinema-arrow-overlay .cinema-scroll-arrow:focus { cursor:pointer ; }
        ...
        html[${ATTR}] body.cinematic-home #contents .cinema-arrow-overlay .cinema-scroll-arrow[disabled] { opacity:.3 ; cursor:default; }
...
function attachCarouselEnhancements() {
  ...
  let candidate = document.querySelector<HTMLElement>(
    'ytd-browse[page-subtype="home"] ytd-rich-grid-renderer #contents.ytd-rich-grid-renderer',
  );
  if (!candidate) {
    candidate = document.querySelector<HTMLElement>(
      'ytd-browse[page-subtype="home"] #contents:has(ytd-rich-item-renderer)',
    );
  }
  if (!candidate) return;
  ...
  carouselContainer.addEventListener("scroll", () => maybeLoadMore(), {
    passive: true,
  });
...
function initSpotlight() {
  ...
  if (
    !document.body.classList.contains("cinema-spotlight-quarter") &&
    !document.body.classList.contains("cinema-spotlight-half")
  ) {
...
function teardownSpotlight() {
  ...
  try {
    spotlightEl.querySelectorAll("iframe").forEach((f) => {
      f.src = "about:blank";
    });
  } catch {
    /* ignore */
...
function isAdOrPromoted(el: HTMLElement): boolean {
  ...
  const badgeText = Array.from(
    el.querySelectorAll(
      "ytd-badge-supported-renderer, .badge-shape-wiz, .badge, .yt-badge",
    ),
  )
...
function pickRandomVideo(): HTMLElement | null {
  ...
  if (items.length > 1 && idx === lastSpotlightIndex)
...
function runSpotlightCycle() {
  ...
  const anchor = target.querySelector<HTMLAnchorElement>(
    'a#thumbnail, a.yt-simple-endpoint[href*="watch"], a#video-title, h3 a[href*="watch"], a[href*="/watch"]',
  );
  const thumbImg = target.querySelector<HTMLImageElement>(
    "ytd-thumbnail img, img#img",
  );

  const backdrop = thumbImg?.src || thumbImg?.getAttribute("data-thumb") || "";
  const titleText = extractText(titleEl, anchor);
  const titleDest = spotlightEl.querySelector<HTMLElement>(
    ".optube-spotlight-title",
  );
  const watchBtn = spotlightEl.querySelector<HTMLAnchorElement>(
    ".optube-spotlight-watch",
  );
...
function observeItemsForSpotlight() {
  ...
  const grid = document.querySelector(
    'ytd-rich-grid-renderer #contents, ytd-browse[page-subtype="home"] #contents',
  );
  if (!grid) return;
  itemsObserver = new MutationObserver(
    debounce((mutsUnknown: unknown) => {
        ...
        m.addedNodes.forEach((n: Node) => {
          ...
          if (n.tagName.toLowerCase() === "ytd-rich-grid-row") {
            ...
            requestAnimationFrame(() => {
              ...
              if (
                !arrowOverlay ||
                !carouselContainer?.contains(arrowOverlay) ||
                !arrowLeft ||
                !arrowRight
              ) {
                addCarouselArrows();
...
function extractText(
  ...
  if (!titleEl && anchor)
    return (anchor.getAttribute("title") || anchor.textContent || "").trim();
  if (!titleEl) return "";
...
function cleanTitle(raw: string): string {
  let t = raw
    ...
    .replace(
      /\b(streamed|premiered)\s+\d+\s+(seconds?|minutes?|hours?|days?|weeks?|months?|years?)\s+ago[,.:;-]*$/i,
      "",
    )
...
function switchSpotlightVideo(videoId: string) {
  if (!spotlightEl) return;
  const frames = spotlightEl.querySelectorAll<HTMLIFrameElement>(
    ".optube-spotlight-video iframe",
  );
  if (!frames.length) return;
  const active =
    Array.from(frames).find((f) => f.classList.contains("active")) || frames[0];
  const next =
    frames.length > 1 ? (frames[0] === active ? frames[1] : frames[0]) : active;
  ...
  if (next.src !== autoplaySrc) next.src = autoplaySrc;
  else next.onload?.(new Event("load"));
}
  ...
  frames.forEach((f) => {
    try {
      f.contentWindow?.postMessage(
        JSON.stringify({
          event: "command",
          func: spotlightMuted ? "mute" : "unMute",
          args: [],
        }),
        "*",
      );
...
function maybeLoadMore() {
  ...
  const nearEnd =
    ...
    carouselContainer.scrollWidth - 800;
  if (!nearEnd) return;
...
function triggerContinuationLoad() {
  const cont = document.querySelector(
    "ytd-continuation-item-renderer, yt-next-continuation, ytd-continuation-renderer",
  );
  if (cont) {
  ...
  if (!arrowLeft || !arrowRight) {
    const mkBtn = (dir: "left" | "right") => {
      ...
      btn.addEventListener("click", () => {
        const delta =
          Math.round(carouselContainer!.clientWidth * 0.8) *
          (dir === "left" ? -1 : 1);
    ...
    carouselContainer.addEventListener("scroll", updateCarouselArrows, {
      passive: true,
    });
...
function updateCarouselArrows() {
  ...
  const scrollable =
    carouselContainer.scrollWidth - carouselContainer.clientWidth > 40;
  if (!scrollable) {
    ...
    const max =
      carouselContainer.scrollWidth - carouselContainer.clientWidth - 4;
    if (sl <= 4) arrowLeft.setAttribute("disabled", "true");
    else arrowLeft.removeAttribute("disabled");
    if (sl >= max) arrowRight.setAttribute("disabled", "true");
    else arrowRight.removeAttribute("disabled");
  }
  try {
    const pad = 18;
